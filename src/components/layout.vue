      ...
      <div class="app-head-inner">
        <div class="head-nav">
          <ul class="nav-list">
            ...
            <li class="nav-pile">{{this.username}}</li>
...
export default {
  ...
  data () {
    return {
      ...
      ifShow:true,
      isLog: false, //用于存储localstorage中的登录状态
      username: '',
      userpassword: '',
      modelbasePath: '',
    }
  ...
  mounted(){
    if(window.localStorage.getItem('USER_ACCOUNT') != null){
      this.isLog = true;
      this.username = window.localStorage.getItem('USER_ACCOUNT');
      this.username = this.username.substring(1,this.username.length-1);
      this.userpassword = window.localStorage.getItem('USER_PASSWORD');
  ...
  methods: {
    myModelBase(){
      const self = this;
      var username = window.localStorage.getItem('USER_ACCOUNT');
      if(username == ""){
        alert("您尚未登录");
      }
      else{
        //self.$router.push(self.modelbasePath);
    ...
    logout(){
      const self = this;
      self.isLog = false;
      this.username = window.localStorage.clear();
      this.ifShow = false;
